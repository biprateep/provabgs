...
if name == "nmf":
    fthetas = [
        os.path.join(
            dat_dir, "fsps.%s.v%s.theta_unt.seed%i.npy" % (name, version, ibatch)
        )
        for ibatch in batches
    ]
else:
    fthetas = [
        os.path.join(dat_dir, "fsps.%s.v%s.theta.seed%i.npy" % (name, version, ibatch))
        for ibatch in batches
    ]
...
fspecs = [
    os.path.join(
        dat_dir,
        "fsps.%s.v%s.lnspectrum.seed%i%s.npy" % (name, version, ibatch, str_wbin),
    )
    for ibatch in batches
]
...
fpca = os.path.join(
    dat_dir,
    "fsps.%s.v%s.seed%i_%i%s.pca%i.hdf5"
    % (name, version, batch0, batch1, str_wbin, n_pca),
)
print(fpca)
